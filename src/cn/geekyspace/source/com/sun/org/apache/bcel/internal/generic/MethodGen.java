...
public class MethodGen extends FieldGenOrMethodGen {
            ...
            if (a instanceof Code) {
                ...
                for (final Attribute cAttribute : cAttributes) {
                    ...
                    if (a instanceof LineNumberTable) {
                        LineNumber[] lineNumbers = ((LineNumberTable) a).getLineNumberTable();
                        for (LineNumber lineNumber : lineNumbers) {
                            final InstructionHandle ih = il.findHandle(lineNumber.getStartPC());
                            if (ih != null) {
                                addLineNumber(ih, lineNumber.getLineNumber());
                            }
                        }
               ...
               AnnotationEntry[] annotationEntries = runtimeAnnotations.getAnnotationEntries();
                for (AnnotationEntry annotationEntry : annotationEntries) {
                    addAnnotationEntry(new AnnotationEntryGen(annotationEntry, cp, false));
                }
            } else {
                addAttribute(a);
            }
    ...
    private CodeException[] getCodeExceptions() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            cExc[i] = c.getCodeException(super.getConstantPool());
        }
        return cExc;
    }
        ...
        if (size > 1) {
            Arrays.sort(lg, new Comparator<LocalVariableGen>() {
                ...
                public int compare(LocalVariableGen o1, LocalVariableGen o2) {
                    return Integer.compare(Integer.valueOf(o1.getIndex()), Integer.valueOf(o2.getIndex()));
                }
            });
        }
        return lg;
